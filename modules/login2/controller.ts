...
export const getLoginView = async (c: Context) => {
    const content = await Deno.readTextFile("./modules/login2/views/content.html");
    const scripts = await Deno.readTextFile("./modules/login2/views/scripts.html");
    ...
    post: async (c: Context) => {
        ...
        try {
          ...
          if (verification.verified) {
            ...
            if (existingDevice) {
              return { error: "Dieses Gerät wurde bereits registriert" };
            }
            ...
            db.query(
              ...
              [
                userId,
                deviceId,
                data.deviceName || 'Unbenanntes Gerät',
                publicKeyBase64
              ]
            );
